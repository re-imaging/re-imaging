...
p1 = catsum %>% filter(tcat > 1000, !is.na(primcat))  %>%  ggplot( aes(x=primcat, y=tcat, fill=primcat)) + geom_col() + coord_flip() +  theme(legend.position = "none") + ggtitle('Relative size of arxiv categories', subtitle = 'Categories with count <1000 not shown')

ggsave(p1, file='../figure/arxiv-cat-relativesize.svg')
