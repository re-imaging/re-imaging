...
ggplot(catsum_filtered, aes(x=primcat, y=tcat, group = topcat, fill=topcat)) + geom_col() + coord_flip() +  theme(legend.position = "right",  axis.text =  element_text(size=14), title = element_text(size = 16)) + ggtitle('Relative size of arxiv categories', subtitle = 'Categories with count <1000 not shown') + labs(fill='Primary discipline', x = 'Sub-field',  y = 'Manuscripts') + scale_x_discrete(limits = rev(levels(catsum$primcat)))
...
ggplot(ccfiltered, aes(x=yr, y=pcat, color = primcat)) + geom_line() + facet_wrap(~primcat) +  theme(legend.position = "none", strip.text = element_text(size=10)) + ggtitle('All primary categories 1991-2018',  '% of publications in a given category appearing each year') + xlab('Year') + ylab('% of overall volume')
